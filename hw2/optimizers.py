...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        # ====== YOUR CODE: ======
        for p, _ in params:
            p.velocity = torch.zeros_like(p) # add velocity as a parameter
        # ========================
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            p.velocity.mul_(self.momentum).sub_(self.learn_rate * dp)
            p.add_(p.velocity)
            # ========================
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        # ====== YOUR CODE: ======
        for p, _ in params:
            p.squared_avg = torch.zeros_like(p)
        # ========================
    ...
    def step(self):
        for p, dp in self.params:
            ...
            dp += self.reg * p
            p.squared_avg.mul_(self.decay).add_((1 - self.decay) * dp.pow(2))
            p.sub_(self.learn_rate / (p.squared_avg.sqrt() + self.eps) * dp)
            # ========================
